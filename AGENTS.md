...
| `src/v2/` | Isolated rewrite lane for the next storage-and-scheduler architecture; current foundation includes `Store`, configuration, instrumentation vocabulary, the schema catalog, table/resource identity descriptors, hierarchical scheduler dependencies, and benchmarkable chunk-capacity planning | `src/v2/mod.rs`, `src/v2/store.rs`, `src/v2/schema.rs`, `src/v2/instrumentation.rs`, `src/v2/command.rs` |
...
- `tests/v2/metadata.rs` verifies schema interning, physical-column ordering, identity policy tagging, hierarchical scheduler resource generation, dependency expansion, and rejection of uncovered logical columns for the rewrite metadata layer.
