...
- Roadmap direction:
  ...
  - Tasks `future/plan/00-foundation.md` through `future/plan/05-schedule-builder.md` are implemented through the isolated `src/v2/` rewrite lane. Treat `src/v2/` as the future-engine boundary and keep it isolated from the current runtime modules.
...
| `src/v2/` | Isolated rewrite lane for the next storage-and-scheduler architecture; current foundation includes `Store`, configuration, instrumentation vocabulary, a table-shape catalog, `Meta` descriptors, packed `Row<'job>` handles, generated `Rows<'job>` views, precomputed `ChunkLayout` values, single-allocation `Chunk` storage, low-level direct table/chunk operations, keyless batched remove recording, typed query descriptors, zip-friendly optional query views, conservative filter-based disjointness analysis validated at `query::all(...)` construction time, monotone dependency-path primitives, and a reusable schedule builder with function families, resolve families, conflict edges, typed insert planning through catalog-backed get-or-create initialization, planned command descriptors, and known-table caching. | `src/v2/mod.rs`, `src/v2/store.rs`, `src/v2/schema.rs`, `src/v2/query.rs`, `src/v2/schedule.rs`, `src/v2/command.rs`, `src/v2/instrumentation.rs` |
...
| `tests/` | Integration suites grouped by engine generation; read these before changing semantics | `tests/v1/check.rs`, `tests/v1/query.rs`, `tests/v1/event.rs`, `tests/v1/derive.rs`, `tests/v2/foundation.rs`, `tests/v2/metadata.rs`, `tests/v2/keyless_rows.rs`, `tests/v2/query_surface.rs`, `tests/v2/schedule_builder.rs` |
| `examples/v2/` | Runnable sample usage for the rewrite lane; keep this aligned with the current `v2` public API | `examples/v2/main.rs`, `examples/v2/store_planning.rs`, `examples/v2/metadata.rs`, `examples/v2/keyless_rows.rs`, `examples/v2/query_surface.rs`, `examples/v2/schedule_builder.rs`, `examples/v2/instrumentation.rs`, `examples/v2/vocabulary.rs` |
...
- `src/v2/` is real compiled code, but it is still only the early rewrite lane through `future/plan/05-schedule-builder.md`. It is not yet feature-parity runtime code.
...
- `examples/v2/` is the executable API-evolution trace for the rewrite lane. It now includes the Task 05 schedule-builder surface. When `v2` public behavior or naming changes, update these examples in the same patch.
