...
- Roadmap direction:
  ...
  - The recommended first-iteration `v2` surface is intentionally small: `Store`, `Configuration`, `ChunkPlan`, `query`, `command`, `Builder`, `Schedule`, `Executor`, `Options`, `Key`, and `Keys`. Lower-level `v2` modules remain available mainly for tests, focused examples, and ongoing rewrite work and should be treated as advanced/internal until the public surface is widened deliberately.
...
| `src/v2/` | Isolated rewrite lane for the next storage-and-scheduler architecture. The recommended first-iteration surface is intentionally narrow: `Store`, `Configuration`, `ChunkPlan`, `query`, `command`, `Builder`, `Schedule`, `Executor`, `Options`, `Key`, and `Keys`. The lane also contains lower-level internals such as the table-shape catalog, `Meta` descriptors, packed `Row<'job>` handles, chunk storage/layout details, dependency-path primitives, schedule-family internals, runtime seeding/trace plumbing, and instrumentation types; those are still compiled and sometimes public for tests and focused examples, but they are not the newcomer-oriented API. | `src/v2/mod.rs`, `src/v2/store.rs`, `src/v2/schema.rs`, `src/v2/key.rs`, `src/v2/query.rs`, `src/v2/schedule.rs`, `src/v2/runtime.rs`, `src/v2/command.rs`, `src/v2/instrumentation.rs` |
| `src/v4/` | Experimental rewrite lane exploring a `Module`-based compose-then-resolve architecture. `Module` trait has 4 methods: `initialize(&self, &mut Store) -> State`, `update(&self, &mut State, &Store) -> bool`, `get(&self, &mut State, &Store) -> Item`, and `resolve(&self, &mut State, &mut Store)` (default no-op). `State<M>` holds module + state + store, returns a `Guard<M>` via `.guard()`. `Guard`'s `next_with(closure)` is the primary safe composition API: items are borrowed during the closure, then the resolved module's `resolve` fires. `insert::Module<T: Template>` and `remove::Module` implement `Module` for deferred insert/remove. See `src/v4/tasks/` for design docs. | `src/v4/mod.rs`, `src/v4/module.rs`, `src/v4/state.rs`, `src/v4/query.rs`, `src/v4/insert.rs`, `src/v4/remove.rs`, `src/v4/template.rs`, `src/v4/table.rs`, `src/v4/tasks/` |
...
- `src/v1/core/borrow.rs` is in the repository but not exported by `src/v1/core/mod.rs`; it is currently not compiled.
...
- `src/v2/` is real compiled code through `future/plan/08-managed-keys.md`, but it is not yet feature-parity runtime code.
...
- `tests/v2/metadata.rs` verifies table-shape interning, `Meta` handling, ordinary `Key` meta handling, hierarchical scheduler resource generation, monotone dependency-path generation, duplicate-meta rejection, `Row` packing, and inline-versus-sidecar layout accounting for the rewrite metadata layer.
...
- `tests/v2/query_surface.rs` verifies typed inline-slice read/write projection, sidecar-column rejection for slice queries, `rows()` plus chunk data alignment, `query::option(...)` semantics, alias-conflict rejection, and one conservative disjoint-filter proof.
- `tests/v2/global_tables.rs` verifies `Store::initialize_global(...)`, standalone `query::one::<T>()` / `query::one_mut::<T>()`, singleton cardinality failure behavior, builder-time rejection of malformed singleton tables, mixed stream-plus-singleton planning, and singleton conflict integration.
- `tests/v2/managed_keys.rs` verifies observable reserved keys, inline keyed insert publication, keyed random lookup, keyed remove release of deleted keys, and reverse-map republishing for moved rows.
- `tests/v2/schedule_builder.rs` verifies monotone dependency-path conflict and coverage rules, empty-path barriers, typed insert get-or-create planning, planned resolve command descriptors, function-to-resolve and resolve-to-resolve ordering edges, known-table caching, known-table dependency refresh after typed insert table creation, and schedule stability across chunk-count changes.
