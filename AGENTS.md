...
- Roadmap direction:
  ...
  - Tasks `future/plan/00-foundation.md` through `future/plan/08-managed-keys.md` are implemented through the isolated `src/v2/` rewrite lane. Treat `src/v2/` as the future-engine boundary and keep it isolated from the current runtime modules.
...
| `src/v2/` | Isolated rewrite lane for the next storage-and-scheduler architecture; current foundation includes `Store`, configuration, instrumentation vocabulary, a table-shape catalog, `Meta` descriptors, packed `Row<'job>` handles, generated `Rows<'job>` views, precomputed `ChunkLayout` values, single-allocation `Chunk` storage, low-level direct table/chunk operations, typed query descriptors, zip-friendly optional query views, conservative filter-based disjointness analysis validated at `query::all(...)` construction time, keyed random lookup through `query::All::get(...)`, monotone dependency-path primitives, a reusable schedule builder with function families, resolve families, conflict edges, explicit `Keys` injection, store-centric initialization for injected capabilities, typed insert planning through `Store::get_or_create_table(...)`, filtered remove planning, per-job command buffers, `RemoveRows`, an atomic-first `Keys` resource with observable reserved/live states, and a frame-local executor runtime seeded from snapshots with worker-local ready queues, work stealing, runtime reports, store-backed batched resolve, inline keyed insert/remove synchronization, and resolve-driven chunk injection. | `src/v2/mod.rs`, `src/v2/store.rs`, `src/v2/schema.rs`, `src/v2/key.rs`, `src/v2/query.rs`, `src/v2/schedule.rs`, `src/v2/runtime.rs`, `src/v2/command.rs`, `src/v2/instrumentation.rs` |
