...
- Roadmap direction:
  ...
  - The selected long-term rewrite direction is now documented in `future/plan/specification.md`, `future/plan/standards.md`, `future/plan/README.md`, and the ordered `future/plan/00-foundation.md` through `future/plan/15-layout-specialization.md` task set. That plan currently assumes per-job command recording, function-level batched resolve phases, monotone dependency paths with optional wildcard identifiers, empty-path full barriers, and resolve-only structural mutation with typed inserts getting or creating their target tables during initialization and storing the resulting table indices in planned resolve metadata. Treat those files as design documents for future work, not descriptions of current runtime behavior.
  - Tasks `future/plan/00-foundation.md` through `future/plan/06-executor-runtime.md` are implemented through the isolated `src/v2/` rewrite lane. Treat `src/v2/` as the future-engine boundary and keep it isolated from the current runtime modules.
...
| `src/v2/` | Isolated rewrite lane for the next storage-and-scheduler architecture; current foundation includes `Store`, configuration, instrumentation vocabulary, a table-shape catalog, `Meta` descriptors, packed `Row<'job>` handles, generated `Rows<'job>` views, precomputed `ChunkLayout` values, single-allocation `Chunk` storage, low-level direct table/chunk operations, keyless batched remove recording, typed query descriptors, zip-friendly optional query views, conservative filter-based disjointness analysis validated at `query::all(...)` construction time, monotone dependency-path primitives, a reusable schedule builder with function families, resolve families, conflict edges, typed insert planning through catalog-backed get-or-create initialization, known-table caching plus dependency refresh when typed inserts create tables, and a frame-local executor runtime with worker-local deques, work stealing, runtime reports, and resolve-driven chunk injection. | `src/v2/mod.rs`, `src/v2/store.rs`, `src/v2/schema.rs`, `src/v2/query.rs`, `src/v2/schedule.rs`, `src/v2/runtime.rs`, `src/v2/command.rs`, `src/v2/instrumentation.rs` |
...
- `src/v2/` is real compiled code, but it is still only the early rewrite lane through `future/plan/06-executor-runtime.md`. It is not yet feature-parity runtime code.
...
- `tests/v2/query_surface.rs` verifies typed inline-slice read/write projection, sidecar-column rejection for slice queries, `rows()` plus chunk data alignment, `query::option(...)` semantics, `query::one(...)` cardinality checks, alias-conflict rejection, and one conservative disjoint-filter proof.
...
- `tests/v2/executor_runtime.rs` verifies seeded per-chunk execution, real stealing across worker-local deques, and resolve-driven same-frame chunk injection for later function families.
...
- `examples/v2/` is the executable API-evolution trace for the rewrite lane. It now includes the Task 06 executor-runtime surface. When `v2` public behavior or naming changes, update these examples in the same patch.
