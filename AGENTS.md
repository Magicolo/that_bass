...
- Roadmap direction:
  ...
  - Tasks `future/plan/00-foundation.md` through `future/plan/07-command-resolution.md` are implemented through the isolated `src/v2/` rewrite lane. Treat `src/v2/` as the future-engine boundary and keep it isolated from the current runtime modules.
...
- `src/v2/` is real compiled code, but it is still only the early rewrite lane through `future/plan/07-command-resolution.md`. It is not yet feature-parity runtime code.
