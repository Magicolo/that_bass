...
// impl<T> RawVec<T> {
//     pub const unsafe fn get_raw_unchecked(this: *const Self, index: usize) ->
// *const T {         unsafe { (*this).ptr.add(index) }
//     }

//     pub const unsafe fn get_mut_raw_unchecked(this: *mut Self, index: usize)
// -> *mut T {         unsafe { (*this).ptr.add(index) }
//     }

//     pub const unsafe fn get_raw(this: *const Self, index: usize) ->
// Option<*const T> {         if index >= unsafe { (*this).len } {
//             None
//         } else {
...
//     pub const unsafe fn get_mut_raw(this: *mut Self, index: usize) ->
// Option<*mut T> {         if index >= unsafe { (*this).len } {
//             None
//         } else {
...
//     pub unsafe fn iter_raw(this: *const Self) -> impl ExactSizeIterator<Item
// = *const T> {         (0..unsafe { (*this).len })
//             .map(move |index| unsafe { Self::get_raw_unchecked(this, index)
// })     }

//     pub unsafe fn iter_mut_raw(this: *mut Self) -> impl
// ExactSizeIterator<Item = *mut T> {         (0..unsafe { (*this).len })
//             .map(move |index| unsafe { Self::get_mut_raw_unchecked(this,
// index) })     }
...
//         let mut vec = unsafe { Vec::from_raw_parts(self.ptr, self.len,
// self.cap) };         vec.push(item);
...
// impl<T> RawSlice<T> {
//     pub const unsafe fn get_raw_unchecked(this: *const Self, index: usize) ->
// *const T {         unsafe { (*this).ptr.cast::<T>().add(index) }
//     }

//     pub const unsafe fn get_mut_raw_unchecked(this: *mut Self, index: usize)
// -> *mut T {         unsafe { (*this).ptr.cast::<T>().add(index) }
//     }

//     pub const unsafe fn get_raw(this: *const Self, index: usize) ->
// Option<*const T> {         if index >= unsafe { (*this).ptr.len() } {
//             None
//         } else {
//             Some(unsafe { Self::get_raw_unchecked(this, index) })
//         }
...
//     pub const unsafe fn get_mut_raw(this: *mut Self, index: usize) ->
// Option<*mut T> {         if index >= unsafe { (*this).ptr.len() } {
//             None
//         } else {
//             Some(unsafe { Self::get_mut_raw_unchecked(this, index) })
//         }
...
//     pub unsafe fn iter_raw(this: *const Self) -> impl ExactSizeIterator<Item
// = *const T> {         (0..unsafe { (*this).ptr.len() })
//             .map(move |index| unsafe { Self::get_raw_unchecked(this, index)
// })     }

//     pub unsafe fn iter_mut_raw(this: *mut Self) -> impl
// ExactSizeIterator<Item = *mut T> {         (0..unsafe { (*this).ptr.len() })
//             .map(move |index| unsafe { Self::get_mut_raw_unchecked(this,
// index) })     }
...
//         let (ptr, len, cap) =
// Vec::into_raw_parts(iter.into_iter().collect::<Vec<_>>());         Self {
//             ptr,
//             len,
//             cap,
//             _marker: PhantomData,
...
//     unsafe fn fragments_raw(this: *const Self) -> impl Iterator<Item =
// Range<usize>> {         let free = unsafe {
// RawVec::iter_raw(addr_of!((*this).free)) };         fragments_iter(
//             free.map(|range| (unsafe { (*range).start }, unsafe {
// (*range).end })),             unsafe { (*this).count },
//         )
//     }
// }
...
//     impl Pointer {
//         #[inline(always)]
//         pub unsafe fn as_ref<'a, T: 'static>(self) -> &'a T {
...
//         #[inline(always)]
//         pub unsafe fn as_mut<'a, T: 'static>(self) -> &'a mut T {
//             debug_assert_eq!(self.meta.identifier(), TypeId::of::<T>());
//             debug_assert_eq!(self.count, 1);
//             unsafe { &mut *self.pointer.cast() }
//         }

//         #[inline(always)]
//         pub unsafe fn as_slice<'a, T: 'static>(self) -> &'a [T] {
//             debug_assert_eq!(self.meta.identifier(), TypeId::of::<T>());
//             unsafe { from_raw_parts(self.pointer.cast(), self.count) }
//         }

//         #[inline(always)]
//         pub unsafe fn as_slice_mut<'a, T: 'static>(self) -> &'a mut [T] {
//             debug_assert_eq!(self.meta.identifier(), TypeId::of::<T>());
//             unsafe { from_raw_parts_mut(self.pointer.cast(), self.count) }
//         }
//     }

//     impl<'a> Context<'a> {
//         pub fn and(&mut self) -> Context<'_> {
//             Self {
//                 pointers: self.pointers,
//             }
//         }

//         pub unsafe fn read<T: 'static>(self) -> &'a [T] {
//             unsafe { self.pointers.pop().unwrap_unchecked().as_slice() }
//         }

//         pub fn write<T: 'static>(self) -> &'a mut [T] {
//             unsafe { self.pointers.pop().unwrap_unchecked().as_slice_mut() }
//         }
//     }
...
//         fn prepare<'a>(&self, context: prepare::Context<'a>) ->
// Self::Item<'a> {             self.0.prepare(context)
//         }
//     }

//     impl<T: Declare + Prepare, F: Fn(T::Item<'_>) + Send + Sync> Task for
// Function<T, F> {         #[inline]
//         unsafe fn run<'a>(&self, mut context: Context<'a>) {
...
//         pub fn push<U: Task>(mut self, task: U) -> Result<Schedule<'a, (T,
// U)>, Box<dyn Error>> {             let resources = Vec::new();
//             // TODO: Declare access to the `Store`, validate and convert into
// a efficient             // format for scheduling...
...
//                             // Branch::Column => todo!(),
//                             Branch::Fragments => (Some(Part::Fragments),
// not),                             Branch::Not => (None, !not),
//                             _ => todo!(),
//                         };
//                         for node in nodes {
//                             descend(node, path.clone().chain(key), resources,
// identifiers, not)?;                         }
//                         Ok(())
//                     }
...
//         pub fn function<I: Declare + Prepare, F: Fn(I::Item<'_>) + Send +
// Sync>(             self,
//             input: I,
//             run: F,
...
//             .function(Query::new().read::<()>().write::<()>().has::<()>(),
// |a| {})?;         for _ in 0..1_000 {
//             schedule.run()?;
//         }
//         Ok(())
...
//         fn prepare<'a>(&self, context: prepare::Context<'a>) ->
// Self::Item<'a> {             self.0.prepare(context.table())
//         }
//     }
