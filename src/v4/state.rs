...
// impl Store {
//     pub fn state<M: module::Module>(&mut self, module: Module<M>) ->
// Result<State<'_, M>, Error> {         Ok(State {
...
//     pub fn get<'b>(&'b mut self) -> Result<<H::Item<'b> as IntoFlat>::Flat,
// Error>     where
//         H::Item<'b>: IntoFlat,
//     {
//         if self.update()? {
...
//     pub fn push<N: IntoNest>(self, modules: N) -> Module<<M as
// Push<N::Nest>>::Out>     where
...
//     fn declare(&self, _: &Self::State, _: &Store) -> impl Iterator<Item =
// Dependency> {         empty()
//     }
...
//     fn update(&self, state: &mut Self::State, store: &mut Store) ->
// Result<bool, Error> {         self.0.update(state, store)
//     }

//     fn get<'a>(&'a self, state: &'a mut Self::State, store: &'a Store) ->
// Self::Item<'a>     where
//         Self: 'a,
//     {
...
//     fn resolve(&self, state: &mut Self::State, store: &mut Store) ->
// Result<(), Error> {         self.0.resolve(state, store)
//     }
// }
...
//     fn declare(&self, state: &Self::State, store: &Store) -> impl
// Iterator<Item = Dependency> {         self.0.0.declare(&state.0, store)
//     }
...
//     fn update(&self, state: &mut Self::State, store: &mut Store) ->
// Result<bool, Error> {         let (head, tail) = self.split_ref();
//         Ok(head.0.update(&mut state.0, store)? | tail.update(&mut state.1,
// store)?)     }
...
//     fn resolve(&self, state: &mut Self::State, store: &mut Store) ->
// Result<(), Error> {         let (head, tail) = self.split_ref();
...
//             Access::Read => None,
//             Access::Write => Some(Error::ReadWriteConflict(resource,
// *entry.key())),         },
//         (Entry::Occupied(entry), Access::Write) => match entry.get() {
//             Access::Read => Some(Error::ReadWriteConflict(*entry.key(),
// resource)),             Access::Write =>
// Some(Error::WriteWriteConflict(*entry.key(), resource)),         },
